...
class MarioEnvironment(Environment) :
    ...
    def __init__(self,game,resized_height=0,resized_width=0,use_pixels=True,is_discrite=False) :
        self.game = game
    ...
    def initialize(self) :
        ...
        if(self.is_discrite) :
            ...
            self.env = BinarySpaceToDiscreteSpaceEnv(self.env, COMPLEX_MOVEMENT)

    def clone(self) :
        return MarioEnvironment(self.game,self.resized_height,self.resized_width,self.use_pixels,self.is_discrite)
    ...
    def actionProcessor(self,action) :
        action = np.reshape(action,newshape=[action.shape[1]])
        if(self.is_discrite) :
