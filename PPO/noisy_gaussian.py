...
class NoisyDense(Layer):
    ...
    def build(self, input_shape):
        ...
        self.stddev_kernel = self.add_weight(shape=(1,),
                                      initializer=initializers.Constant(value=self.sigma_init),
                                      name='stddev_kernel')
        ...
        if self.use_bias:
            ...
            self.sigma_bias = self.add_weight(shape=(self.units,),
                                        initializer=initializers.Constant(value=self.sigma_init),
                                        name='sigma_bias')
            self.stddev_bias = self.add_weight(shape=(1,),
                                        initializer=initializers.Constant(value=self.sigma_init),
                                        name='stddev_bias')
        else:
            self.bias = None
    ...
    def call(self, X):
        if(self.training) :
            perturbation = self.sigma_kernel * K.random_normal(shape=(self.input_dim, self.units), mean=0, stddev=self.stddev_kernel)
            perturbed_kernel = self.kernel + perturbation
            output = K.dot(X, perturbed_kernel)
            if self.use_bias:
                bias_perturbation = self.sigma_bias * K.random_normal(shape=(self.units,), mean=0, stddev=self.stddev_bias)
