...
def create_conv2d_layers(input,hidden_layers,hidden_size,hidden_activation,flatten=True) :
    ...
    for i in range(hidden_layers) :
        x = tf.layers.conv2d(x,filters=hidden_size[i],kernel_size=3,strides=1,activation=hidden_activation)
...
'''
    input.shape = [None,input_size]
'''
def create_mlp_network(input,hidden_layers,hidden_size,hidden_activation,output_size,output_activation,output_name=None) :
    x = input
...
,mlp_hidden_layers,mlp_hidden_size,mlp_hidden_activation,output_size,output_activation,output_name) :
...
'''
    input.shape = [None,height,width,channels]
'''
def create_network_pixels_mlp(input,hidden_layers,hidden_size,hidden_activation,output_size,output_activation,output_name) :
    input_shape = input.get_shape().as_list()
    x = input
