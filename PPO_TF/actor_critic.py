...
class ActorCritic() :
    def __init__(self,sess,state_size,action_size,is_discrete) :
    ...
    def init(self,writer=None) :
        ...
        if(Config.actor_critic_type == 'seperate') :
            params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='actor') + tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='critic')
        else :
            params = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='actor_critic')
        self.saver = tf.train.Saver(params)
        if(Config.start_timestep > 0) :
            self.load()

    def load(self) :
        dir = Config.root_dir + '/models/timestep-' + str(Config.start_timestep) + '/actor_critic/model.ckpt-' + str(Config.start_timestep)
        
        self.saver.restore(self.sess,dir)

    def save(self,dir,current_timestep) :
        self.saver.save(self.sess,dir + '/actor_critic/model.ckpt',global_step=current_timestep)
    ...
    def build_critic(self,base_network) :
        if(Config.actor_critic_type == 'seperate') :
